            ...
            if (db_infos.user_infos.length == 0) { // da registrare
                ...
                if (comands.length <= 0) {
                    ...
                    if (splitted_text.length > 1) {
                        ...
                        visualizzazione_bool = (visualizzazione_triggers.indexOf(splitted_text[1].split("#").join("")) >= 0);
                    }
                ...
                if (options_array[3] == "CMDS") {
                    ...
                    return model.getUserDaft(user.id).then(async function (inc_struct) {
                        ...
                        if (paragraph_infos.esit == false) {
                            ...
                            if (typeof inc_struct.cached_paragraphs_infos == "undefined") {
                            ...
                            for (let i = 0; i < inc_struct.cached_paragraphs_infos.length; i++) {
                                if (inc_struct.cached_paragraphs_infos[i].id == paragraph_infos.id){
                                    inc_struct.cached_paragraphs_infos[i].title = paragraph_infos.choice_title;
                                    break;
                                }
                            }

                            await model.setUserTmpDaft(user.id, inc_struct);
                            await model.updateParagraph(user.id, paragraph_infos.id, paragraph_infos);
...
function set_adventureTitle_message(inc_struct, paragraph_infos, user, new_title, option) {
    ...
    if (typeof new_title != "string" || new_title.length <= 5 || new_title.length > 80) {
        ...
        if (typeof paragraph_infos.id != "undefined") {
        ...
        message_text += "• Precedente: `" + inc_struct.title + "`\n";
        buttons_array[0].unshift({ text: "↵", callback_data: 'B:TMP:EDIT:TITLE' }, { text: "✓", callback_data: 'B:TMP:OPTION_CONFIRM:TITLE' });
    } else if (option == 2) {
        message_text += "_del paragrafo " + paragraph_infos.id + "_\n\n";
        message_text += "• Nuovo: " + new_title + "\n";
        message_text += "• Precedente: `" + paragraph_infos.choice_title + "`\n";
        buttons_array[0].unshift({ text: "↵", callback_data: 'B:TMP:EDIT:TITLE:'+ paragraph_infos.id }, { text: "✓", callback_data: 'B:TMP:PRGPH:CH_TITLE:' + paragraph_infos.id });

    } else {
        message_text += "_nel paragrafo " + paragraph_infos.id + "_\n\n";
        message_text += "• Nuovo: " + new_title + "\n";

        buttons_array.push(
            [{ text: "📜 Avventura", callback_data: 'B:TMP:EDIT:TITLE:'+ paragraph_infos.id+":1" }],
            [{ text: "⨓ Paragrafo", callback_data: 'B:TMP:EDIT:TITLE:'+ paragraph_infos.id+":2" }]
        );
...
function selectParagraph(user, inc_struct, page_n) {
    ...
    if (inc_struct.paragraphs_ids.length != inc_struct.cached_paragraphs_infos.length) {
        message_text += "\n*∿ Fuori sync* " + inc_struct.cached_paragraphs_infos.length + "/" + inc_struct.paragraphs_ids.length + "\n• Segnala a @nrc382\n";
        buttons_array[0].push({ text: "↺", callback_data: "B:TMP:LIST_UPDATE:" });
    }
        ...
        if (new_paragraph_text.length == 0) {
            ...
            if (type == 2) {
                ...
                if (paragraph_infos.text == "" && paragraph_infos.night_text == "") { // ☀️
                    ...
                    message_text = mess_title + "_del paragrafo " + paragraph_infos.id + "_\n\n" + to_save + mess_options;
                }
            } else {
                if (type == 0) {
                    if (paragraph_infos.text != "") {
...
function paragraph_setTex_confirm(user, inc_struct, type, query_text) {
    return new Promise(async function (paragraph_setTexConfirm_res) {
        ...
        if (inc_struct.paragraphs_ids.indexOf(user.has_pending) < 0) {
            ...
            let loaded_paragraph_infos = await model.loadParagraph(user.id, user.has_pending);
            ...
            if (type == 0) {
                ...
                loaded_paragraph_infos.text += "\n" + new_paragraph_text;
            } else if (type == 4) { // integra notturno
                loaded_paragraph_infos.night_text += "\n" + new_paragraph_text;
            }

            let paragraph_update = await model.updateParagraph(user.id, user.has_pending, loaded_paragraph_infos);
            if (paragraph_update.esit === false) {
                to_return = paragraph_update;
            } else {
                to_return = { paragraph_infos: loaded_paragraph_infos };
            }
        }
    ...
    return new Promise(async function (paragraph_addChoice_confirm_res) {
        ...
        if (inc_struct.paragraphs_ids.indexOf(curr_paragraph_id) < 0) {
            ...
            if (repeat_counter > 1) {
                ...
                message_text += "\n\nTesto in input:\n> `" + newChoice_text + "`\n";
                let to_return = simpleMessage(message_text, user_id, [[{ text: "⨓ ", callback_data: "B:TMP:PRGPH:SELECT:" + paragraph_infos.id }, { text: "⨷", callback_data: "B:FORGET" }]]);
            ...
            if (paragraph_update.esit === false) {
                ...
                if (!inc_struct.cached_paragraphs_infos) {
                    ...
                    for (let i = 0; i < inc_struct.cached_paragraphs_infos.length; i++) {
                        if (inc_struct.cached_paragraphs_infos[i].id == paragraph_infos.id) {
                            inc_struct.cached_paragraphs_infos[i].title = new_choice.title_text;
                            inc_struct.cached_paragraphs_infos[i].level_deep = (paragraph_infos.level_deep + 1);
                            inc_struct.cached_paragraphs_infos[i].availability = new_choice.availability;
                            break;
                        }
                    }
                }
...
function paragraph_setChoiceText_message(user_id, paragraph_infos, new_choice_text, curr_index) {
    ...
    if (paragraph_infos.choices.length <= 0) {
        ...
        message_text = "➽" + " *Modifica Strada*\n_ …nel paragrafo " + paragraph_infos.id + "_\n\n";
        ...
        let buttons_array = [[{ text: "⨓ ", callback_data: "B:TMP:PRGPH:SELECT:" + paragraph_infos.id + ":TO_SEND" }, { text: "⨷", callback_data: "B:FORGET" }]];
    ...
    return new Promise(async function (paragraph_setChoiceText_confirm_res) {
        ...
        if (inc_struct.paragraphs_ids.indexOf(user.has_pending) < 0) {
            ...
            if (!inc_struct.cached_paragraphs_infos) {
                ...
                for (let i = 0; i < inc_struct.cached_paragraphs_infos.length; i++) {
                    if (inc_struct.cached_paragraphs_infos[i].id == child_paragraph_infos.id) {
                        inc_struct.cached_paragraphs_infos[i].title = child_paragraph_infos.choice_title;
                        inc_struct.cached_paragraphs_infos[i].level_deep = child_paragraph_infos.level_deep;
                        inc_struct.cached_paragraphs_infos[i].availability = child_paragraph_infos.availability;
                        break;
                    }
                }
            }
...
function paragraph_setDelay_message(user_id, inc_struct, paragraph_infos, new_delay, choice_index) {
    ...
    console.log("Paragrafo: " + paragraph_infos.id);
    ...
    if (isNaN(new_delay) || (new_delay != 0 && (new_delay < 2) || new_delay > 90)) {
        ...
        if (choice_index == "DEFAULT") { // Per il default
            ...
            if (inc_struct.delay) {
                ...
                if (inc_struct.delay > 60) {
                    message_text += "(1h";
                    if ((inc_struct.delay - 60) == 1 || (inc_struct.delay - 60) == 8 || (inc_struct.delay - 60) == 1) {
                        message_text += " ed " + (inc_struct.delay - 60) + "m)";
                    } else if ((inc_struct.delay - 60) > 0) {
                        message_text += " e " + (inc_struct.delay - 60) + "m)";
                    }
                }
                message_text += "\n";
            }
...
function paragraph_setChoiceDelay_confirm(user, cmds_array, inc_struct) {
    return new Promise(function (paragraph_setChoiceDelay) {
        ...
        } else if (inc_struct.paragraphs_ids.indexOf(user.has_pending) < 0) {
            message_text = "*Woops!*\n\n";
            message_text += "Non mi risulta che " + paragraph_id + " sia l'id di un paragrafo della tua bozza...";
            return ({ esit: false, text: message_text });
        } else {
...
function paragraph_setOptions_message(user_id, inc_struct, paragraph_infos) {
    ...
    if (!inc_struct.paragraphs_ids || inc_struct.paragraphs_ids.indexOf(paragraph_infos.id) < 0) {
        ...
        if (paragraph_infos.father_id == 0) {
            ...
            message_text = "⌥" + " *\"" + paragraph_infos.choice_title + "\"*\n";
            message_text += "_#opzioni paragrafo " + paragraph_infos.id + "_\n\n";
            ...
            buttons_array.push([
                { text: "☠", callback_data: 'B:TMP:PRGPH:CH_ESIT:' + paragraph_infos.id },
            ]);
...
function paragraph_setChoiceAvailability_manager(user, in_query, options_array) {
    return new Promise(async function (setAv_res) {
        ...
        const update_res = await model.updateParagraph(user.id, paragraph_infos.id, paragraph_infos);
        const father_update_res = await model.updateParagraph(user.id, father_paragraph_infos.id, father_paragraph_infos);
...
function paragraph_Variation_manager(user, inc_struct, paragraph_infos, variation, option) {
    ...
    if (curr_state.title == "" || typeof curr_state.title == "undefined") {
        ...
        if (paragraph_infos.availability == "ALL" && paragraph_infos.id != inc_struct.paragraphs_ids[0]) {
...
function paragraph_buttons_manager(paragraph_infos, counters, inc_struct) {
    ...
    for (let i = 0; i < paragraph_infos.choices.length; i++) { // Scelte
        ...
        let condition = paragraph_infos.choices[i].availability == "ALL" || (inc_struct.view_type == "ALL") || (paragraph_infos.choices[i].availability == inc_struct.view_type);
        if (condition) {
            let this_callback = "";
            if (paragraph_infos.choices[i].is_alternative == true) {
