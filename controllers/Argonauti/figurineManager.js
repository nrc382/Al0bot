...
let pvp_type = new Array(figurine);
let rarityInfo = {
    uno: 0,
    due: 0,
    tre: 0,
        ...
        if (typeof message.text == "undefined") { //|| message.chat.id != theCreator){
            ...
            if (message.chat.id == edicolaID && message_array.length > 0 || message_array[0] == "cerco" || message_array[0] == "scambio") {
                ...
                if (message_array[0] === "genera") {
                    ...
                    first_res.toSend = simpleMessage(res_text, message.chat.id);
                    res.push(first_res);
                } else if (message_array[0] === "pvp") {
                    let partialNames_text = messages_title + "\nCi sono " + pvp_type.length + " tipi *base per pvp*:\n\n";
                    let ordered_list = pvp_type.sort();
        ...
        preloadInfos().then(function (cards_infos) {
            ...
            if (generalStuff.cards_count <= 0 || (now_date - generalStuff.last_cards_update) > 60 * 60 * 24 * 7) {
                ...
                request({
                    ...
                    rarityInfo = allFiltered(allCards);

                    let not_primary = ["Gigantesco", "Piccolo", "Bavoso", "Fetido", "Piccola", "Grande", "Strana", "Incantevole"];
                    for (let i = 0; i < allCards.length; i++) {
                        if (not_primary.indexOf(allCards[i].partial_name) >= 0) {
                            if (pvp_type.indexOf(allCards[i].attribute.toLowerCase()) < 0) {
                                pvp_type.push(allCards[i].attribute.toLowerCase());
                            }
                        } else if (pvp_type.indexOf(allCards[i].partial_name.toLowerCase()) < 0) {
                            pvp_type.push(allCards[i].partial_name.toLowerCase());
                        }
                    }
