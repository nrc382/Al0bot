...
const test_oggetti = require("../../LootBot/LootBot/logic/items");
const bot_response = require("../../LootBot/LootBot/utility/bot_response");
const lab = require("../../LootBot/LootBot/message_managers/specific/master_craftsman");
const nd = require("../../LootBot/LootBot/message_managers/specific/necro_descent");
...
argo_controller.update().then(async function (argo_res) {
	try {
		...
		// init di test
		let init2 = await test_oggetti.init();
		console.log(init2);


		let card_res = await cards_controller.loadAllCards(false)
		...
		/*
		let edicola_res = await cards_controller.loadEdicolaStuff()
		if (!edicola_res) {
		...
		*/
		...
		let webHook_res = await al0_bot.setWebHook('https://www.al0.eu/' + config.path + '/post', options);
		//console.log(await al0_bot.getWebHookInfo());
...
al0_bot.on('callback_query', async function (query) {
	...
	if (query_crossroad == "CRAFTSMAN") {
		let response = await lab.queryDispatcher(query);
		return bot_response.manage(response, al0_bot);
	} else if (query_crossroad == "NECRO_D") {
		let response = await nd.queryDispatcher(query);
		return bot_response.manage(response, al0_bot);
...
al0_bot.on("message", async function (message) {
	...
	if (typeof message.text != 'undefined') {
		...
		if (first_word == "/viscere") {
			...
			return simpleSend(res);
		} else if (first_word == "/altare") {
			let response = await nd.menu(message.chat.id);
			return bot_response.manage(response, al0_bot);
		} else if (first_word == "/altarebeta") {
			let response = await nd.add_betaTester(message.from.id, message.text);
			return bot_response.manage(response, al0_bot);
		} else if (first_word == "/craftbeta") {
			let response = await lab.add_betaTester(message.from.id, message.text);
			return bot_response.manage(response, al0_bot);
		} else if ("mastro artigiano ⚒".indexOf(message.text.toLowerCase()) >= 0) {
			let response = await lab.menu(message.chat.id);
			return bot_response.manage(response, al0_bot);
